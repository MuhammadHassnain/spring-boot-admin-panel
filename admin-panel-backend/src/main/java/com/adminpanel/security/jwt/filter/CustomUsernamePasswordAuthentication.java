...
public class CustomUsernamePasswordAuthentication extends UsernamePasswordAuthenticationFilter{
	...
	public CustomUsernamePasswordAuthentication(JwtManager jwtManager,AuthenticationManager manager,UserService userService, AdminService adminService) {
		super();
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		resContent.put("user_info", user);
		
		switch (user.getUserType().getUserTypeName()) {
		case "ADMIN":
			resContent.put("user_detail", adminService.findAdminById(user.getId()));
			break;
	...
	protected void unsuccessfulAuthentication(HttpServletRequest request, HttpServletResponse response,
		...
		if(failed instanceof BadCredentialsException) {
			ApplicationException applicationException = ApplicationException.builder(HttpStatus.UNAUTHORIZED, "Username or password is incorrect");
			throw applicationException;
		}else if (failed instanceof AccountStatusException) {
			
			ApplicationException applicationException = ApplicationException.builder(HttpStatus.UNAUTHORIZED, "account is not accessible");
			applicationException.setNextAction("Please Contact customer support at info@adminpanel.com");
			throw applicationException;
			
		}else {
			ApplicationException applicationException = ApplicationException.builder(HttpStatus.UNAUTHORIZED, "Please Try Again");
			applicationException.setNextAction("Please Contact customer support at info@adminpanel.com, for frequent error");
			throw applicationException;
		}
